...
BEGIN_MESSAGE_MAP(CDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BT_DENY, &CDlg::OnBnClickedBtDeny)
	ON_COMMAND(ID_MENU_CONNECTION, &CDlg::OnMenuConnection)
	ON_COMMAND(ID_MENU_QUITTER, &CDlg::OnMenuQuitter)
END_MESSAGE_MAP()
...
BOOL CDlg::OnInitDialog()
{
	...
	m_pED_LOG->ReplaceSel(_T("Veuillez vous connecter à un appareil\r\n"
		"Allez dans 'Menu' puis 'Connection'\r\n"
		"---------------------------------------------------------------------------------------------------\r\n"));
...
//----- Refuser l'accès à la porte -----//
void CDlg::OnBnClickedBtDeny()
{
}
...
//Ouvre la fenêtre de connection aux appareils bluetooth
void CDlg::OnMenuConnection()
{
	//DialogBox(NULL, MAKEINTRESOURCE(IDD_CONNECTION_DIALOG), NULL,NULL);
	CDlgConnection dlgPupitre;
	dlgPupitre.DoModal();
}


void CDlg::OnMenuQuitter()
{
	OnCancel();
}
